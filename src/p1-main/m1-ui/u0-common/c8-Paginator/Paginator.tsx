...
import Button from "../c2-Button/Button"; // TODO: resolve external component dependency
...
export const Paginator: FC<TPaginatorProps> = ({
    ...
    for (let i = 1; i <= pagesCount; i++) {
        ...
        else if ((i <= 6 && currentPage <= 3)  // while one of the first three pages is active
            || i === pagesCount       // pagination will be: 1 2 3 4 5 6 last
            || i === 1) {
            pagesVisibleSet.push(i)
        }
        else if ((i > pagesCount - 6 && currentPage >= pagesCount - 3) // while one of the last three pages is active
    ...
    const pages = pagesVisibleSet.map((p: number) => (
        currentPage === p
            ? <Button key={p}
                        variant={"active"}
                        style={{minWidth: '45px', fontSize: '13px',}}><b>{p} </b></Button>
            : <Button key={p}
                        variant={"inactive"}
        ...
        <div className={css.paginator__container}>
            <div className={css.paginator}>
                <Button onClick={() => setCurrentPage(currentPage-1)}
                        ...
                        variant={"active"}
                        style={{minWidth: '45px', fontSize: '13px',}}><b>{'◄'}</b></Button>
                {pages}
                <Button onClick={() => setCurrentPage(currentPage+1)}
